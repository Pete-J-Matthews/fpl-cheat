...
def process_manager_search(
    q: str, current_manager_id: int | None
    ...
    if q and q.strip():
        if len(q.strip()) >= 2:
            ...
            st.info(
                "💡 Enter at least 3 characters to search, or enter your manager ID (numbers only) directly"
            )
    ...
    if q and len(q.strip()) >= 2:
        st.info(
            "No matches found. Try a different search term or enter your manager ID directly (numbers only)."
        )
...
def get_manager_id_from_choice(
    choice: str, placeholder: str = "--- Select a manager ---"
) -> int | None:
...
def run_compare_if_needed(
    ...
    if (
        ...
        or st.session_state.comparison_results is None
    ):
        with st.spinner("Comparing teams..."):
            live_gameweek = get_current_event_id_cached()
            creator_teams = get_creator_teams_cached()
            creator_teams = [
                t for t in creator_teams if t.get("current_gameweek") == live_gameweek
            ]
...
def render_similar_teams() -> dict | None:
    ...
    with st.container(key="similar_teams_pills"):
        ...
        cols = st.columns(3)
        for idx, (creator_team, similarity) in enumerate(
